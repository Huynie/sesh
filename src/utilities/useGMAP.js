...
import { spotsData } from "./apiCalls";
...
const useGMAP = (
  ...
  const getMap = useCallback(async () => {
    const newMap = await new window.google.maps.Map(mapRef.current, {
      center,
      zoom,
  ...
  const setAutocomplete = useCallback(
    async (locationType, ref, handlePlaceSelect) => {
      if (map && ref.current) {
        const newAutocomplete =
          await new window.google.maps.places.Autocomplete(ref.current, {
            componentRestrictions: { country: "us" },
            types: [locationType],
            fields: ["address_components", "formatted_address", "geometry"],
          });
        if (handlePlaceSelect) {
          newAutocomplete.addListener(
            "place_changed",
            handlePlaceSelect(map, newAutocomplete)
          );
        }
      }
    },
  ...
  useEffect(() => {
    const loadFirst = async () => {
      await getMap();
      setGeocoder(await new window.google.maps.Geocoder());

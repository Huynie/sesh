...
const SideBar = React.forwardRef(({ data }, ref) => {
  ...
  return (
    <div ref={sidebarRef} className={`sidebar ${sidebarShow}`}>
      ...
      <Tooltip target="sidebarToggle" isOpen={tooltipToggle} placement="left">
        Please pick a spot!
      </Tooltip>
      <Card
      ...
      >
        <CardBody style={{ flex: 0 }}>
          <CardTitle tag="h5" className="text-center mb-3">
            {data?.name ? data.name : "Spot Name"}
          </CardTitle>
          <CardText tag="p" className="fs-6">
          ...
          <CardText tag="p">
            Liability Waiver: {data?.waiver ? "required" : "not required"}
          </CardText>
        ...
        <Accordion
          flush
          toggle={(index) =>
            setAccordionId(`${accordionId === "1" ? "0" : index}`)
          }
          open={accordionId}
        >
          <AccordionItem>
            ...
            <AccordionBody accordionId="1">
              {data &&
                Object.keys(data.hours).map((hour, idx) => {
